...
public class TaiKhoanDAO implements DAOinterface<TaiKhoanDTO> {
    ...
    public boolean updatePass(String email, String newPasswordHash) {
        boolean result = false;
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "UPDATE taikhoan SET matkhau=? WHERE tendangnhap=?"; // Thay email thành tendangnhap
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, newPasswordHash);
            pst.setString(2, email); // Sử dụng tendangnhap thay vì email

            int rowsAffected = pst.executeUpdate();
            if (rowsAffected > 0) {
                result = true; // Cập nhật mật khẩu thành công
            }
        ...
        return result; // Trả về true nếu mật khẩu được cập nhật thành công, false nếu không
    }
    ...
    public TaiKhoanDTO selectByManv(int manv) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                tk = new TaiKhoanDTO(manv, username, matkhau, manhomquyen, trangthai);
            }
