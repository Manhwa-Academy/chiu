...
public class XuatXuDialog extends JDialog implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == add) {
            ...
            if (index != -1) {
                String tenxuatxu = ms.getText();
                // Kiểm tra nếu tên xuất xứ không trống
                if (Validation.isEmpty(tenxuatxu)) {
                    JOptionPane.showMessageDialog(this, "Vui lòng nhập nơi xuất xứ mới");
                } else {
                    // Kiểm tra trùng lặp với các xuất xứ khác
                    if (msBUS.checkDup(tenxuatxu)) {
                        // Cập nhật xuất xứ
                        ...
                        JOptionPane.showMessageDialog(this, "Xuất xứ đã tồn tại vui lòng kiểm tra lại!");
                    }
                }
            }
        } else if (e.getSource() == table) {
            // Khi chọn một dòng trong bảng, điền tên xuất xứ vào ô nhập liệu
