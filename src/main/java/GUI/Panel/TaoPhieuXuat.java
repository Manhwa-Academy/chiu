...
public final class TaoPhieuXuat extends JPanel {
    ...
    private void initComponent(String type) {
        ...
        String[] header = {
                "STT", "Mã SP", "Tên sản phẩm", "Phiên bản", "Đơn giá", "Số lượng"
        };
        ...
        // Duyệt các cột và căn chỉnh
        for (int i = 0; i < header.length; i++) { // Lặp qua số lượng cột thực tế (header.length)
            if (i != 2) { // Cột thứ 2 (Tên sản phẩm) không căn giữa
    ...
    public void loadDataTableChiTietPhieu(ArrayList<ChiTietPhieuDTO> ctPhieu) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            tblModel.addRow(new Object[] {
                    i + 1,
                    phienban.getMasp(),
                    spBUS.getByMaSP(phienban.getMasp()).getTensp(),
                    phienban.getTenphienban(),
