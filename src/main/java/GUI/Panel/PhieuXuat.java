...
public final class PhieuXuat extends JPanel
    ...
    private void initComponent() {
        ...
        String[] header = new String[] { "STT", "Mã phiếu xuất", "Khách hàng", "Nhân viên nhập", "Thời gian",
                "Tổng tiền" };
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // Kiểm tra mainFunction.btn và pxBUS có null không
        if (mainFunction == null || mainFunction.btn == null) {
            System.out.println("mainFunction or mainFunction.btn is null");
            return;
        }
        ...
        if (source == mainFunction.btn.get("create")) {
            if (m != null && tk != null) {
                ...
                JOptionPane.showMessageDialog(null, "Lỗi: Không thể tạo phiếu xuất!");
            }

        } else if (source == mainFunction.btn.get("detail")) {
            int selectedRow = getRow(); // Get the selected row
            if (selectedRow < 0) {
                JOptionPane.showMessageDialog(null, "Vui lòng chọn phiếu cần xem!");
            } else if (pxBUS != null) {
                PhieuXuatDTO selectedPhieuXuat = pxBUS.getSelect(selectedRow);
                if (selectedPhieuXuat != null) {
                    new ChiTietPhieuDialog(m, "Thông tin phiếu xuất", true, selectedPhieuXuat);
                } else {
                    JOptionPane.showMessageDialog(null, "Lỗi: Không tìm thấy phiếu xuất hoặc chỉ số không hợp lệ!");
                }

            } else {
                JOptionPane.showMessageDialog(null, "Lỗi: pxBUS is null!");
            }

        } else if (source == mainFunction.btn.get("cancel")) {
            // Lấy tất cả các dòng được chọn
            int[] selectedRows = tablePhieuXuat.getSelectedRows();

            if (selectedRows.length == 0) {
                JOptionPane.showMessageDialog(null, "Vui lòng chọn ít nhất một phiếu để xóa!");
                return;
            }

            // Hiển thị số lượng phiếu sẽ xóa
            String message = "Bạn có chắc muốn xóa " + selectedRows.length + " phiếu đã chọn?";

            // Xác nhận xóa các phiếu đã chọn
            int n = JOptionPane.showConfirmDialog(null, message, "Xóa phiếu", JOptionPane.YES_NO_OPTION);

            if (n == JOptionPane.YES_OPTION) {
                // Lặp qua các dòng được chọn và xóa chúng
                for (int row : selectedRows) {
                    if (pxBUS != null) {
                        PhieuXuatDTO px = pxBUS.getSelect(row);
                        if (px != null) {
                ...
                Notification notification = new Notification(m, Notification.Type.SUCCESS,
                        Notification.Location.TOP_CENTER, "Hủy các phiếu thành công");
    ...
    // Cập nhật phương thức getRows() để hỗ trợ chọn nhiều dòng
    public int[] getRows() {
        return tablePhieuXuat.getSelectedRows();
    }
    ...
    public void Fillter() throws ParseException {
        if (validateSelectDate()) {
            ...
            int makh = cbxKhachHang.getSelectedIndex() == 0 ? 0
                    : khachHangBUS.getByIndex(cbxKhachHang.getSelectedIndex() - 1).getMaKH();
            ...
            this.listPhieuXuat = pxBUS.fillerPhieuXuat(type, input, makh, manv, time_start, time_end, min_price,
                    max_price);
