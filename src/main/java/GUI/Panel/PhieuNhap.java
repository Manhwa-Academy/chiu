...
public final class PhieuNhap extends JPanel
    ...
    public int getRowSelected() {
        ...
        if (index == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn phiếu nhập", "Thông báo", JOptionPane.WARNING_MESSAGE);
        }
        return index; // Trả về -1 nếu không có phiếu nhập nào được chọn
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source == mainFunction.btn.get("create")) {
            ...
            if (index != -1 && index < listPhieu.size()) { // Kiểm tra index có hợp lệ không
                PhieuNhapDTO selectedPhieu = listPhieu.get(index);
                if (selectedPhieu != null) { // Kiểm tra phiếu nhập có hợp lệ không
                    new ChiTietPhieuDialog(m, "Thông tin phiếu nhập", true, selectedPhieu);
                } else {
                    JOptionPane.showMessageDialog(null, "Phiếu nhập không hợp lệ!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(null, "Vui lòng chọn một phiếu để xem chi tiết!", "Lỗi",
                        JOptionPane.ERROR_MESSAGE);
            }
        } else if (source == mainFunction.btn.get("cancel")) {
            // Lấy tất cả các dòng được chọn
            int[] selectedRows = tablePhieuNhap.getSelectedRows(); // Sử dụng getSelectedRows() để lấy nhiều dòng được
                                                                   // chọn

            if (selectedRows.length == 0) {
                JOptionPane.showMessageDialog(null, "Vui lòng chọn ít nhất một phiếu để hủy!");
                return;
            }

            // Hiển thị số lượng phiếu sẽ hủy
            String message = "Bạn có chắc muốn hủy " + selectedRows.length + " phiếu đã chọn?";

            // Xác nhận hủy các phiếu đã chọn
            int n = JOptionPane.showConfirmDialog(null, message, "Hủy phiếu", JOptionPane.YES_NO_OPTION);

            if (n == JOptionPane.YES_OPTION) {
                // Lặp qua các dòng được chọn và hủy chúng
                for (int row : selectedRows) {
                    if (row >= 0 && row < listPhieu.size()) {
                        PhieuNhapDTO pn = listPhieu.get(row);
                        if (pn != null) {
                            // Kiểm tra nếu phiếu này có thể hủy được hay không
                            if (!phieunhapBUS.checkCancelPn(pn.getMaphieu())) {
                                JOptionPane.showMessageDialog(null,
                                        "Phiếu này không thể hủy do sản phẩm đã được xuất!");
                            } else {
                                // Hủy phiếu nhập
                ...
                List<PhieuNhapDTO> allPhieuNhap = phieunhapBUS.getAll();
                if (allPhieuNhap != null && !allPhieuNhap.isEmpty()) {
                    loadDataTalbe(new ArrayList<>(allPhieuNhap)); // Chuyển List thành ArrayList
                } else {
                    JOptionPane.showMessageDialog(null, "Không có phiếu nào để hiển thị!", "Lỗi",
                            JOptionPane.ERROR_MESSAGE);
                }
            }
