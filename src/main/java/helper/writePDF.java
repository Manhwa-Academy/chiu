...
public class writePDF {
    ...
    public writePDF() {
        try {
            // Đặt các font trong thư mục lib
            String fontPath = new File("lib/TimesNewRoman/SVN-Times New Roman.ttf").getAbsolutePath();
            fontNormal10 = new Font(BaseFont.createFont(fontPath, BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 12,
                    Font.NORMAL);

            fontPath = new File("lib/TimesNewRoman/SVN-Times New Roman Bold Italic.ttf").getAbsolutePath();
            fontBold25 = new Font(BaseFont.createFont(fontPath, BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 25,
                    Font.NORMAL);

            fontPath = new File("lib/TimesNewRoman/SVN-Times New Roman Bold.ttf").getAbsolutePath();
            fontBold15 = new Font(BaseFont.createFont(fontPath, BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 15,
                    Font.NORMAL);

            fontPath = new File("lib/TimesNewRoman/SVN-Times New Roman Italic.ttf").getAbsolutePath();
            fontBoldItalic15 = new Font(BaseFont.createFont(fontPath, BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 15,
                    Font.NORMAL);
    ...
    public void writePN(int maphieu) {
        ...
        try {
            ...
            Paragraph company = new Paragraph("Quản lý bán mô hình", fontBold15);
            ...
            for (ChiTietPhieuDTO ctp : ChiTietPhieuNhapDAO.getInstance().selectAll(maphieu + "")) {
                ...
                if (sp != null) {
                    ...
                    table.addCell(new PdfPCell(new Phrase("Sản phẩm không tìm thấy", fontNormal10)));
                }

                PhienBanSanPhamDTO pbsp = new PhienBanSanPhamDAO().selectById(ctp.getMaphienbansp());
                if (pbsp != null) {
                    table.addCell(new PdfPCell(new Phrase(pbsp.getTenphienban(), fontNormal10)));
                } else {
                    table.addCell(new PdfPCell(new Phrase("Phiên bản không tìm thấy", fontNormal10)));
                }
        ...
        try {
            ...
            for (ChiTietPhieuDTO ctp : ChiTietPhieuXuatDAO.getInstance().selectAll(maphieu + "")) {
                ...
                table.addCell(new PdfPCell(new Phrase(
                        pbsp.getTenphienban(), // Sử dụng tên phiên bản
                        fontNormal10)));
