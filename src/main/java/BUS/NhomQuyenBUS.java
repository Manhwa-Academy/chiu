...
public class NhomQuyenBUS {
    ...
    public boolean add(String nqdto, ArrayList<ChiTietQuyenDTO> ctquyen) {
        // Lấy mã nhóm quyền tự động tăng từ DAO
    ...
    public boolean checkPermisson(int maquyen, String chucnang, String hanhdong) {
        ...
        boolean check = false;
        int i = 0;
        while (i < ctquyen.size() && !check) {
            // Kiểm tra nếu có quyền với chức năng và hành động đã cho
    ...
    public ArrayList<NhomQuyenDTO> search(String text) {
        ...
        // Tìm kiếm nhóm quyền theo mã hoặc tên
        for (NhomQuyenDTO i : this.listNhomQuyen) {
            if (Integer.toString(i.getManhomquyen()).contains(text)
                    || i.getTennhomquyen().toLowerCase().contains(text.toLowerCase())) {
                result.add(i);
            }
        }
        return result;
