...
public class PhieuNhapBUS {
    ...
    public int cancelPhieuNhap(int maphieu) {
        // Kiểm tra xem phiếu có tồn tại trong cơ sở dữ liệu hay không
        PhieuNhapDTO phieuNhap = phieunhapDAO.selectById(Integer.toString(maphieu));
        if (phieuNhap == null) {
            System.out.println("Phiếu nhập không tồn tại!");
            return 0; // Trả về 0 nếu không tìm thấy phiếu
        }

        // Lấy danh sách chi tiết phiếu nhập
        ArrayList<ChiTietPhieuNhapDTO> arrCt = ChiTietPhieuNhapDAO.getInstance().selectAll(Integer.toString(maphieu));
        if (arrCt.isEmpty()) {
            System.out.println("Không có chi tiết phiếu nhập nào để hủy!");
            return 0; // Trả về 0 nếu không có chi tiết phiếu nhập
        }

        // Lặp qua các chi tiết phiếu nhập và cập nhật số lượng tồn
        for (ChiTietPhieuNhapDTO chiTietPhieuNhapDTO : arrCt) {
            // Kiểm tra và cập nhật số lượng tồn cho từng sản phẩm
            PhienBanSanPhamDTO pb = PhienBanSanPhamDAO.getInstance().selectById(chiTietPhieuNhapDTO.getMaphienbansp());
            if (pb != null) {
                ...
                System.out.println("Phiên bản sản phẩm không tồn tại!");
                return 0; // Trả về 0 nếu phiên bản sản phẩm không tồn tại
            }
        }
        ...
        if (result == 0) {
            System.out.println("Hủy phiếu nhập không thành công!");
        } else {
            System.out.println("Hủy phiếu nhập thành công!");
        }
